...
class Engine {
  ...
  _loop(now) {
    ...
    // Flüssig-Modus: kostet ein Bild wenig und fällt nichts aus, läuft die Vorschau mit 60 Bildern pro Sekunde
    // (Kamerafahrten gleiten sichtbar ruhiger); sobald es eng wird, zurück auf 30 – vor jeder Auflösungsstufe.
    const cap = this._fps60 ? 1000 / 60 - 2 : 1000 / 30 - 3;
    if (!this.exportMode && now && this._lastDraw && now - this._lastDraw < cap) { this._raf = requestAnimationFrame(this._loop); return; }
    ...
    this._cost = this._cost == null ? cost : this._cost * 0.9 + cost * 0.1;
    if (!this._no60 && !this._fps60 && this._cost < 4.5 && this.scale >= this.playMax && ++this._cheap > 45) { this._fps60 = true; this._dts = []; }
    else if (this._fps60 && this._cost > 8) { this._fps60 = false; this._cheap = 0; this._dts = []; }
    else if (!this._fps60 && this._cost >= 4.5) this._cheap = 0;
