...
function planOnce(opts) {
  ...
  if ((s.burst === 'drop' || rush || level >= 2) && !flight && goodMedia(usable, allOn).length >= 5) {
    ...
    if (level >= 4) for (const b0 of (an.barStart || []).map((x) => x - win.start)) if (b0 > introEnd + barDur && b0 < D - 3 && !peaksB.some((y) => Math.abs(y.rel - b0) < barDur * 2.9)) peaksB.push({ rel: b0 });
    peaksB.sort((x, y) => x.rel - y.rel);
    for (const pk of peaksB.slice(0, level >= 4 ? 40 : level >= 3 ? 6 : level >= 2 ? 3 : 1)) {
      const sub = beatDur / 2 >= 0.2 ? beatDur / 2 : beatDur;
      const ds = pk.rel, de = Math.min(D - 1.2, ds + sub * (level >= 3 ? 16 : 8));
  ...
  if (chrono) {
    ...
    let res = layoutChrono(segs, queue);
   ...
   */
  function layoutChrono(segs0, queue) {
    ...
    const remaining = (i) => {
      ...
      return img - Math.max(0, (nT - vT) / Math.max(0.3, avg)) - cap;
    };
    let repeatPtr = 0;
    const repeatImg = () => {
    ...
    for (let i = 0; i < sg.length; i++) {
      ...
      if (head.kind === 'video') {
        ...
        let j = i, end = g.end;
        while (end - g.start < w * 0.95 && j + 1 < sg.length && absorb(sg[j + 1]) && sg[j + 1].end - g.start <= w * 1.2) { j++; end = sg[j].end; }
        const nx = sg[j + 1];
        if (end - g.start < w * 0.9 && absorb(nx) && nx.end - g.start <= Math.min(w * 1.3, videoSpan(head) / 0.8) && nx.end - g.start - w < w - (end - g.start)) { j++; end = nx.end; }
        // Speed-Ramp: läuft ein Video in einen Drop hinein, endet sein Platz genau auf dem Einsatz (beschleunigt hinein)
        if (s.ramp === 'drop') {
          for (let k = j - 1; k >= i; k--) {
            const pkStart = sg[k].end;
            if (pkStart - g.start >= minW(head) && isPeakSec(sectionAt(an, win.start + pkStart + 0.02)) && !isPeakSec(sectionAt(an, win.start + pkStart - 0.05))) { j = k; end = pkStart; break; }
          }
        }
      ...
      const needSplit = s.split !== 'off' && need > 0.5 && beatsIn >= (level >= 2 ? 0.9 : 1.8) && (peak || level >= 1);
