...
- `npm test` – Lint + schnelle Tests (flow, style, videos, allmedia, ui, e2e, features), eine Zeile je Test.
...
| `plan/media.js` | Tempo-Kurven, `videoPlay`, Reihenfolge (`flowOrder`), `imageMotion` |
...
| `director.js` | Auto-Regie: Länge, Songausschnitt, Tempo, Look, Einstieg/Ende, Stil-Budget (`rs.*`) |
| `renderer.js` | WebGL-Shader (zwei Ebenen A/B, Grading, Übergänge, Farbmomente) |
| `overlay.js` | Titel, Kapitel, Karten, Countdown usw. (Canvas 2D, ungegradet) |
